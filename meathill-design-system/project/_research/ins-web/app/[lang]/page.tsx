...
export default async function HomePage({ params }: Props) {
  ...
  // FAQ 限额数字从 PLAN_LIMITS 渲染，避免字典写死过时数字。所有 a 都跑 fillTemplate
  // 兜底（无占位符无副作用），防止部分语言 q/a 编号错位（如 de.json 把限额放 a10）。
  const planVars = {
  ...
  const webAppJsonLd = {
    ...
    url: process.env.NEXT_PUBLIC_SITE_URL || 'https://instagramvideodownload.net',
  ...
  return (
    <main>
      ...
      <section className="mx-auto max-w-4xl px-6 pb-16 pt-16 text-center sm:pt-24">
        <h1 className="mb-5 text-4xl font-extrabold tracking-tight sm:text-5xl lg:text-6xl">{dict.hero.title}</h1>
        <p className="mx-auto mb-10 max-w-3xl text-lg text-muted-foreground leading-relaxed">{dict.hero.subtitle}</p>
      ...
      <section className="mx-auto max-w-5xl px-6 pb-14">
        <h2 className="mb-10 text-center text-3xl font-bold tracking-tight">{dict.features.title}</h2>
      ...
      <div className="mx-auto max-w-5xl px-6">
        <PremiumCta lang={lang} title={dict.premium.title} subtitle={dict.premium.subtitle} cta={dict.premium.cta} />
      </div>
