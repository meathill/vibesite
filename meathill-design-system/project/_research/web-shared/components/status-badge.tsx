...
export function StatusBadge({
  ...
  const label = value === 'running' && stage && labels[stage] ? labels[stage] : labels[value] || value;

  return (
    <Badge variant={config.variant} className={cn('rounded-full px-2.5 py-0.5', config.extra, className)}>
      {label}
    </Badge>
  );
}
