...
import { XLogo, YoutubeLogo, InstagramLogo } from '@phosphor-icons/react/ssr';
    ...
    <footer className="mx-auto max-w-5xl border-t border-border/20 px-6 py-12">
      ...
      {otherSites.length > 0 && (
        <div className="mb-8 text-center">
          <h3 className="mb-3 text-sm font-semibold text-muted-foreground/80">All Useful Downloaders You Need</h3>
          <div className="flex flex-wrap items-center justify-center gap-4">
            {otherSites.map((site) => {
              ...
              return (
                ...
                >
                  {IconComponent && <IconComponent size={18} weight="fill" style={{ color: siteIcon.color }} />}
                  {site.name}
                </a>
              );
            })}
      ...
      <div className="flex flex-col items-center gap-4 text-center text-sm text-muted-foreground/60">
        ...
        <div className="flex items-center gap-4">
          <Link href={`${langPrefix(lang)}/pricing`} className="hover:text-foreground transition-colors">
            {navDict.pricing}
          </Link>
          <span>&middot;</span>
          <Link href={`${langPrefix(lang)}/terms`} className="hover:text-foreground transition-colors">
            {dict.terms}
          </Link>
          <span>&middot;</span>
          <Link href={`${langPrefix(lang)}/privacy`} className="hover:text-foreground transition-colors">
            {dict.privacy}
          </Link>
          <span>&middot;</span>
          <Link href={`${langPrefix(lang)}/contact`} className="hover:text-foreground transition-colors">
            {dict.contact}
          </Link>
        </div>
        <p className="text-xs">
