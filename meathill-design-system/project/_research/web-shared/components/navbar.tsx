...
import { GlobeIcon, CaretDownIcon, XLogoIcon, YoutubeLogoIcon, InstagramLogoIcon } from '@phosphor-icons/react/ssr';
import type { Icon } from '@phosphor-icons/react';
import { NavbarUserStatus } from './navbar-user-status';
import { langPrefix, SUPPORTED_LANGS, type Lang } from '../lib/i18n';
      ...
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-6">
        <div className="flex items-center gap-6">
          <Link href={langPrefix(lang) || '/'} className="flex items-center gap-2.5">
            <Image src="/logo.svg" alt={logoAlt} width={36} height={36} className="rounded-xl shadow-lg" />
              ...
              <div className="absolute left-0 top-full z-50 mt-1 min-w-48 rounded-xl border border-border/40 bg-background p-1 shadow-lg">
                {SITES.map((site) => {
                  ...
                  return isCurrent ? (
                    ...
                    >
                      {IconComponent && <IconComponent size={16} weight="fill" style={{ color: siteIcon.color }} />}
                      {site.shortName}
                    </span>
                  ) : (
                    <a
