...
export function PricingCards({ initialInterval, features, loginHref, texts, loginDict }: PricingCardsProps) {
      ...
      <div className="grid gap-8 sm:grid-cols-2">
            ...
            {features.map((feat) => (
              <li key={feat.name} className="flex items-center gap-3 text-sm">
                ...
                <span className={feat.free === false ? 'text-muted-foreground/60' : ''}>
                  {feat.name}: {feat.free === false ? texts.no : feat.free === true ? texts.yes : feat.free}
                </span>
              </li>
            ))}
          </ul>
        ...
        <div className="relative rounded-3xl border-2 border-primary/30 bg-gradient-to-b from-primary/5 to-transparent p-8 shadow-xl shadow-primary/5">
          <div className="absolute -top-3 left-1/2 -translate-x-1/2">
            <span className="rounded-full bg-primary px-4 py-1 text-xs font-bold text-primary-foreground">POPULAR</span>
          </div>
