...
function BatchActions({ count, onEmoji, onAITemplate, onApprove, onReject, onDelete }) {
  return (
    <div className="batchbar">
      <div className="selcount">Selected <b>{count}</b> comments</div>

      <div className="group">
        <span className="grouplbl">Quick reply</span>
        <button className="emoji-btn" onClick={() => onEmoji('❤️')} title="Heart">❤️</button>
        <button className="emoji-btn" onClick={() => onEmoji('👍')} title="Thumbs up">👍</button>
        <button className="emoji-btn" onClick={() => onEmoji('😂')} title="Laugh">😂</button>
        <button className="emoji-btn" onClick={() => onEmoji('🙏')} title="Thanks">🙏</button>
        <button className="emoji-btn" onClick={() => onEmoji('🎉')} title="Celebrate">🎉</button>
      </div>
