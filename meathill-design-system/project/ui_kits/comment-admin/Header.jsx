    ...
    <header className="header">
      <div className="header-inner">
        <div className="brand">
          ...
          <b>Meathill</b>
          <span className="product"><i className="ph ph-chat-circle-dots"></i> Comments</span>
        </div>
        <nav className="header-tabs">
          {tabs.map((t) => (
            <a key={t.key} href="#" className={tab === t.key ? 'active' : ''} onClick={(e) => e.preventDefault()}>
              <i className={`ph ${t.i}`}></i>{t.label}
            </a>
          ))}
        </nav>
        <div className="header-right">
          <button className="btn btn-md btn-ghost" title="Notifications" style={{ width: 36, padding: 0 }}>
            <i className="ph ph-bell"></i>
          </button>
          <ThemeToggle />
          <div className="avatar" title="meathill@example.com">M</div>
        </div>
      </div>
    </header>
  );
