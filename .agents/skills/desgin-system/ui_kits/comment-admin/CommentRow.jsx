      ...
      : tag === 'Bug report'
        ...
        : tag === 'Criticism'
          ...
          : tag === 'Suggestion'
            ...
            : 'greeting';
  return (
    <button className={`tag ${cls}`} onClick={() => onFilter?.(tag)}>
      {tag}
    </button>
  );
}
  ...
  return (
    <tr className={(selected ? 'selected ' : '') + (c.isForeign ? 'foreign' : '')}>
      ...
      <td className="content-cell">
        ...
        {c.translation && (
          <div className="translation">
            <b>
              <i className="ph ph-translate"></i> {c.lang ? `${c.lang} → en` : 'translation'}
            </b>
            {c.translation}
          </div>
        )}
            ...
            {c.replies.map((r, idx) => (
              <div key={idx} className={'reply' + (r.byAI ? ' ai' : '')}>
                ...
                <div className="reply-meta">
                  <span className="by">
                    <i className="ph ph-patch-check"></i>
                    {r.author || 'You'}
                  </span>
                  {r.byAI && (
                ...
                <div className="reply-actions">
                  <button>Edit</button>
                  <button className="danger" onClick={() => onRemoveReply(c.id, idx)}>
                    Delete
                  </button>
                </div>
              </div>
        ...
        {(!c.replies || c.replies.length === 0) && (
          <div className="reply-composer">
            ...
            <button className="emoji-btn" onClick={() => onReply(c.id, '❤️')} title="Heart">
              ❤️
            </button>
            <button className="emoji-btn" onClick={() => onReply(c.id, '👍')} title="Thumbs up">
              👍
            </button>
            <button className="emoji-btn" onClick={() => onReply(c.id, '😂')} title="Laugh">
              😂
            </button>
            <button className="emoji-btn" onClick={() => onReply(c.id, '🙏')} title="Thanks">
              🙏
            </button>
            ...
            <button className="write">
              <i className="ph ph-pencil-simple"></i> Write
            </button>
          </div>
        )}
      </td>
      ...
      <td className="post-cell">
        <div className="post">
          ...
          <div className="post-tools">
            <button title="Filter by this post">
              <i className="ph ph-funnel"></i>
            </button>
            <button title="Filter by slug">
              <i className="ph ph-link-simple"></i>
            </button>
            <a href={c.postId} target="_blank" rel="noreferrer noopener" title="Open post">
      ...
      <td className="tags-cell">
        ...
        {c.lang && c.lang !== 'en' && (
          <button className="tag lang">
            <i className="ph ph-globe"></i> {c.lang}
          </button>
        )}
      </td>
        ...
        <div className="status-stack">
          ...
          {c.shadowBanned && (
            <span className="pill shadow">
              <i className="ph ph-eye-slash"></i> banned
            </span>
          )}
        </div>
      </td>
