...
import { CrownIcon, GearIcon, SpinnerGapIcon } from '@phosphor-icons/react';
...
import type { DownloadResponse, DownloadTask } from '../types/download';
  ...
  async function fetchData(): Promise<void> {
    ...
    try {
      const [tasksResult, usageResult] = await Promise.allSettled([
        fetch('/api/downloads?limit=50').then(
          (r) => r.json() as Promise<{ ok: boolean; items?: DownloadTask[] }>,
        ),
        fetch('/api/user/usage').then(
          (r) =>
            r.json() as Promise<{ ok: boolean; used: number; limit: number; remaining: number }>,
        ),
      ]);

      if (
        tasksResult.status === 'fulfilled' &&
        tasksResult.value.ok &&
        Array.isArray(tasksResult.value.items)
      ) {
          ...
          <div>
            ...
            {usage && (
              <p className="mt-1 text-xs text-muted-foreground">
                {usage.limit === -1
                  ? dict.unlimited
                  : `${usage.remaining}/${usage.limit} ${dict.remainingToday}`}
              </p>
            )}
          </div>
        </div>
