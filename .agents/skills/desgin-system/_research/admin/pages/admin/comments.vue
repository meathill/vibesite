...
const {
  ...
  async () => {
    ...
    const [cms, replies, replyTo]: [Record<number, RowItem>, Comment[], Record<string, Comment>] = (
      data || []
    ).reduce(
      ([map, replies, replyTo], c) => {
        ...
        if (
          ...
          [CommentStatus.Pending, CommentStatus.Approved, CommentStatus.Rejected].includes(
            filterStatus.value,
          )
        ) {
