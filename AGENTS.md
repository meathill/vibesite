...
> 本文件与 `AGENTS.md` / `CLAUDE.md` / `GEMINI.md` 三份等效，以 `AGENTS.md` 为准，修改时请三份同步。
...
本项目为 Concierge 上线服务，无通用模型/API 接入流程。新接口按 `PRD.md §11` 设计：先定路由与错误码（`lib/api-response.ts` 的 `apiError/apiOk`），前后端共享校验（`lib/validation.ts` 的 zod schema），再补测试与文档。
...
- 使用 pnpm
- 使用 Next.js + OpenNext 构建，部署到 Cloudflare Workers
- 使用 vitest 编写测试
